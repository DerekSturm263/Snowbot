...
export function build_new_pet(pet, isEgg, isDead) {
	...
	return new EmbedBuilder()
		...
		.setTitle(`${isEgg ? '🥚 Unhatched Egg' : `${pet.icon} ${pet.name}${isDead ? ' (RIP)' : ''}`}`)
...
export function build_new_pet_unlocked(pet) {
	return new EmbedBuilder()
        ...
        .setDescription(`**<t:${Math.floor(pet.hatch_time / 1000)}:R>**, the egg will hatch into a new pet! Come back later and use \`/pets\` to see what it is! After your pet is born, don't forget to feed it at least once a day!`)
